...
const _bubbleTo = Symbol('bubbleTo');
...
module.exports = class {
    ...
    trigger (event) {
        ...
        if (this[_channels].has(event.type)) {

            // iterate over the callbacks and call them one by one
            for(let callback of this[_channels].get(event.type)) callback(event);
        }

        // should we bubble the event further?
        if (this[_bubbleTo]) this[_bubbleTo].trigger(event);
    ...
    on(name, callback) {
        ...
        if (!this[_channels].has(name)) this[_channels].set(name, []);

        // push next callback on the
        this[_channels].get(name).push(callback);
    ...
    off(name, callback) {
        ...
        // do we even have a channel?
        if (!this[_channels][name]) return this;

        // filter the callbacks
        this[_channels].set(name, this[_channels].get(name).filter(function (item) {
